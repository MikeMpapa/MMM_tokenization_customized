...
def preprocess_music21_songs(songs, train):
    ...
    for song in songs:
        try:
            ...
            logger.error(f"Could not preprocess song: {song.metadata.title}")
            logger.error(e)
            continue
    return songs_data
...
def preprocess_music21_song(song, train):
    ...
    #print(song_data["title"], song_data["genre"], song_data["artist"],song_data["number"],first_part)
    # Get time signature of first measure of first part
    #print(first_part.recurse().getElementsByClass(meter.TimeSignature))
